...
{phang}
{opt drop} drops the created controls once the wrapped command has run. The default keeps them.
...
them. The diagnostics are computed on the sample the controls were built on.
...
estimator for the fixed-effects coefficients computed from the fixed-effects residual, with no
small-sample adjustment, and posts those coefficients with z-based inference. The pooled
regression is kept in {cmd:e(cre_b_pooled)} and {cmd:e(cre_V_pooled)}. Each {it:vcetype}
corresponds to an assumption on the disturbance e once the fixed effects are removed.

{phang2}
{opt white} is the heteroskedasticity-robust estimator, valid when the disturbances are
independent across observations. Its error is of the order of the number of fixed effects per
observation, so that it can understate the standard errors when that ratio is not small.
{opt robust} is a synonym.
...
independent with a constant variance. It requires the exact projector, and so is available only
when the number of fixed-effect levels outside the largest dimension is at most {opt dcap()};
the largest dimension can be of any size. Observations without within variation, such as
singletons, are dropped, and their number is reported.

{phang2}
{opt union} clusters on all absorbed dimensions at once. It allows dependence between any two
observations sharing a category of any absorbed dimension, for instance a component common to
all observations of a store-brand pair. It is the multiway cluster-robust estimator of
Cameron, Gelbach, and Miller (2011) with the absorbed dimensions as the clustering variables.
...
cell of two or more dimensions, the estimator reduces to the homoskedastic one, and the output
says so.
...
relative to that within variation, posted in {cmd:e(cre_gap_k)}. The last of the three governs
how far the dimension-wise estimates fall from the fixed-effects ones, since a small gap moves
them a long way when the fixed effects leave little of the regressor. The gap is not computed
with weights.
...
dimension is absorbed exactly at no cost, so that the only dense object is a square matrix of
that order; a panel with 100,000 firms and 200 industry-years has 200 such levels. Above the
cap the rank is taken from {helpb reghdfe}, which equals the exact rank when singletons are kept
and can overstate it when three or more dimensions are absorbed, and {opt fevce(lc)} and
{opt fevce(plugin)} are not available. The default is 10000.

{phang}
{opt memcap(#)} sets, in doubles, the memory allowed to the one dense object of
{opt fevce(plugin)}, at most the number of observations times the number of levels plus
regressors. The default is 5e7, about 400 MB.
...
category, but not the components shared by observations with two or more categories in common,
nor any other dependence. {opt white} assumes independence. {opt union} and {opt plugin} allow
...
clusters are not too large relative to the sample. In practice, {opt union} and {opt cluster()}
on the dimensions along which shocks are shared are the estimators of interest, and {opt white}
...
is small relative to the sample size. The condition is sufficient and not necessary, so that its
failure does not show the standard errors to be invalid; their validity is not verified by this
check.

{pstd}
{it:Note: the clustered variance was not positive semidefinite.} Clustering on overlapping
dimensions adds and subtracts cell sums, and the result can fail to be positive semidefinite in
a finite sample; the negative eigenvalues are set to zero. The event becomes rare as the sample
grows and does not affect the validity of the tests.
...
outside the largest dimension exceeds {opt dcap()}, so that the rank was not computed exactly;
with three or more dimensions it can be overstated. Raise {opt dcap()} to compute it exactly.
...
Mata library on first use if needed. No object of the size of the sample squared is formed: the
controls come from {helpb reghdfe}, the cell sums from grouped passes over the data, and the
...
estimators, the diagnostics and the test were validated against an independent implementation
on the designs of the paper's Monte Carlo experiments, to machine precision on the point
...
the created controls in the data, and so is not available after {opt drop}.
